...
- **Фильтры списков — только `components/Filters.tsx`** (`FilterBar` + `FilterSelect`):
  одно измерение — один компактный список, выбранный отмечен акцентной рамкой,
  «Сбросить» появляется, когда фильтр что-то отсекает. Измерение с одним
  вариантом фильтр скрывает сам. Рядами чипов фильтры не делать: три измерения
  превращались в три ряда залитых «таблеток», которые перебивали сам список.
  `Pill` остаётся для переключателей режима (План/Архив) и выбора значения
  в настройках
