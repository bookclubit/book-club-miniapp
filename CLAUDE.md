...
- `/book/:bookId` — Book (`bookId` = имя папки книги, напр. `docker-up-and-running`):
  ...
  (глав бывает больше десятка, поэтому под заголовком списка — ряд ссылок-номеров
  `.chapter-jump`). Колонка узкая (`max-w-3xl`): в широкой строка темы растягивается
  и оценка у правого края висит в пустоте. Якоря — `#<slug главы>` и `#<id темы>`
  (на них ведут «Топ» и профиль спикера)
...
- `books/{folder}/chapters/{chapterSlug}/chapter.json` — глава целиком, вместе с темами:
  ...
  выводятся на странице книги блоками (`components/ChapterTopics.tsx`): заголовок
  «номер · название · число тем», под ним темы строками
...
Вид как на Reddit: стрелка вверх, баланс голосов, стрелка вниз. Баланс
...
- Длинный список группируется блоками-карточками с заголовком (глава книги —
  `components/ChapterTopics.tsx`): элемент внутри остаётся строкой, но десяток
  групп без рамок читается одной простынёй
