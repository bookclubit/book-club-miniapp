...
- `/book/:bookId` — Book (`bookId` = имя папки книги, напр. `docker-up-and-running`):
  ...
  Якоря — `#<slug главы>` и `#<id темы>` (на них ведут «Топ» и профиль спикера)
...
- `books/{folder}/chapters/{chapterSlug}/chapter.json` — глава целиком, вместе с темами:
  ...
  выводятся списком на странице книги (`components/ChapterTopics.tsx`), подпись
  «Глава N · название» — вертикально у левого края, чтобы не отбирать место у тем
...
- Вертикальный текст — только через `.chapter-label` в `index.css` (подпись главы
  на странице книги): `writing-mode` в утилитах Tailwind нет, а произвольные
  значения запрещены
