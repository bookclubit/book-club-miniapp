...
- `/book/:bookId` — Book (`bookId` = имя папки книги, напр. `docker-up-and-running`):
  ...
  `#<id темы>` (на них ведут «Топ» и профиль спикера)
...
- `/meetings` — Meetings: таймлайн встреч. Разделы «План» и «Архив» — вкладки
  ...
  остался на главной
- `/top` — Top: рейтинг тем по всем книгам. Темы берутся из тех же глав, что и
  профиль спикера (ключ SWR `chapters-all`), оценки — из D1 бота
  (`lib/ratings.ts`, ключ `topic-ratings:<id аккаунта>`)
...
Тему оценивают в один тап — «полезно» или «не очень» (`components/TopicVote.tsx`
в строке темы и в «Топе»). Голос привязан к аккаунту, поэтому без входа кнопки
выключены с подсказкой, но счётчик виден всем.

Состояние общее (`useTopicRatings` в `lib/ratings.ts`, один ключ SWR): оценка,
поставленная в главе, сразу меняет рейтинг во вкладке «Топ». Обновление
оптимистичное, `score` не считаем на клиенте — рейтинг (нижняя граница Уилсона)
приходит от бота, формула должна быть одна.

Вид как на Reddit: стрелка вверх, баланс голосов, стрелка вниз — столбиком
**у левого края строки**, чтобы у всех тем страницы была одна линия стрелок
и глаз не ходил к правому краю и обратно. Баланс
(«полезно» минус «не очень») может быть отрицательным, но на порядок в «Топе»
он не влияет — там рейтинг Уилсона, поэтому один минус тему не роняет.
